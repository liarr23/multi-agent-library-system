...
async def get_agents_status():
    ...
    for name, agent in coordinator.agents.items():
        # 确保agent有这些属性
        processed_count = getattr(agent, '_processed_count', 0)
    ...
    return {
        "coordinator": {
            ...
            "message_history_count": len(coordinator._message_history),
            "task_count": coordinator._task_counter
        },
...
async def get_message_history(limit: int = 50):
    ...
    messages = []
    for msg in coordinator._message_history[-limit:]:
        messages.append({
            "id": msg.message_id,
            "sender": msg.sender,
    ...
    return {"messages": messages, "total": len(coordinator._message_history)}
